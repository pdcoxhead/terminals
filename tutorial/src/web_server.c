...
const char *terminals  = "<html><body>What terminals would you like?</body></html>";  // Fake it till you make it.
const char *errorpage =
  "<html><body>This doesn't seem to be right.</body></html>"; //tutorial cruft
...
const MethodEndpointMap post_endpoint_maps[] = {
  ...
  {"",       &error_post_notfound}, //Keep the error page as the last in the array
};
              ...
              void **con_cls)
{
  ...
  for (i = 0; i < ep_map_size - 1; i++) { // size - 1 because last handler is an error page
    ...
    // Could use tolower to make sure it's case insenstive for the url and method
    // It would be better to check that it's of the form endpoint/.* somehow
    if (strncmp((const char*)map->endpoint, (const char*)url, sizeof(map->endpoint)) == 0) {
...
{
  // Extract the id of the requested terminal from the url '/endpoint/id'
  char* savePtr;
  char* urlCopy = (char*)malloc(sizeof(char) * (strlen(url) + 1));
  strncpy(urlCopy, url, strlen(url));
  char* endpoint_base = strtok_r(urlCopy, "/", &savePtr);
  printf("endpoint_base '%s'\n", endpoint_base);
  char* endpoint_parameter = strtok_r(NULL, "/", &savePtr);

  printf("endpoint_parameter '%s'\n", endpoint_parameter);
  long result = BAD_ID;
  if ( NULL != endpoint_parameter) {
  	result = atol(endpoint_parameter);
  }
...
int get_terminals(struct MHD_Connection *connection,
                   ...
                   size_t *upload_data_size, void **con_cls)
{
  long id = get_id_from_url(url);
  if (id != BAD_ID) {
  	printf("someone asked for terminal %ld\n", id);
  } else {
  	printf("get the whole bunch of terminals\n");
  }
  struct MHD_Response *response;
